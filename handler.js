    ...
    try {
      ...
      try {
        ...
        if (user) {
            ...
            if (!isNumber(user.lastkyubiclaim)) user.lastkyubiclaim = 0
            ...
            if (!isNumber(user.nagalasclaim)) user.nagalastclaim = 0
            ...
            if (!isNumber(user.lastngepett)) user.lastngepett = 0
            if (!isNumber(user.lastgrab)) user.lastgrab = 0
            if (!isNumber(user.lastngojek)) user.lastngojek = 0
            if (!isNumber(user.lastdagang)) user.lastdagang = 0
            if (!isNumber(user.lastcodereg)) user.lastcodereg = 0
            if (!isNumber(user.lastpremcode)) user.lastpremcode = 0
            if (!isNumber(user.lasthunt)) user.lastbansos = 0
            if (!isNumber(user.lastpremhack)) user.lastpremhack = 0
...
`.trim())
          ...
          if (!isPrems && plugin.limit && global.DATABASE._data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}blimit* 3500Xp Per Limit`, m)
        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user Patuhi Rulesnya Dulu Ya!').replace('@subject', await this.getName(jid)).replace('@desc', groupMetadata.desc) :
                (chat.sBye || this.bye || conn.bye || 'Bye, @user! Jangan Balik Lago Ya Beban Alam Semesta')).replace('@user', '@' + user.split('@')[0])
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Perintah ini hanya dapat digunakan oleh _*REAL OWWNER!1!1!*_',
    owner: 'Perintah ini hanya dapat digunakan oleh _*Owner Bot*_!',
    mods: 'Cuman Moderator Only!, Perintah ini hanya dapat digunakan oleh _*Moderator*_ !',
    premium: 'Emang Lu Premium Member Hah?!, Perintah ini hanya untuk member _*Premium*_ !',
    group: 'Perintah ini hanya dapat digunakan di grup!',
    //unreg: ' *「  BELUM TERDAFTAR 」* \nHalo Kak, Yuk Daftar Dulu Karena Anda Belum Terdaftar Dalam Database Bot\n\nKetik : #daftar nama.umur\nContoh : #daftar Botz Whatsapp.17',
    private: 'Perintah ini hanya dapat digunakan di Chat Pribadi!',
    admin: 'Emang Lu Admin Hah!?, Perintah ini hanya untuk *Admin* grup!',
    botAdmin: 'Bot Belum Admin Tod!, Jadikan bot sebagai *Admin* untuk menggunakan perintah ini!',
  }[type]
