...
let handler = async ( m, { conn, args, command}) => {
  ...
  let enemy = global.DATABASE._data.users[who]
  ...
  let timers = clockString(_timers)
   ...
   try {
     ...
     let pler = `@${m.sender.replace(/@.+/, '')} Mengajak duel ${args[0]}\n\nKetik .dya Untuk Menerima Tantangan Atau Ketik .dno Untuk Tolak Tantangan`
     let mentionedJid = [m.sender]

       if (new Date - user.lastduel > 300000) {
      conn.sendButton(m.chat, pler, `Botz Whatsapp`, `.dya`, `.dno`, `No`, `+dno`, m, false, { contextInfo: { mentionedJid }})
