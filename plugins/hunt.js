...
let handler = async (m, { conn, text }) => {
  ...
  if (new Date - global.DATABASE._data.users[m.sender].lasthunt > 120000) {
    ...
    let pesan = `*@${pengirim}* Menemukan Dan Membunuh *${monsterName}*\nMendapatkan ${new Intl.NumberFormat('en-US').format(coins)} money & ${new Intl.NumberFormat('en-US').format(exp)} XP\nBerkurang -${dmg}Hp, Tersisa ${player.healt}/${100}`
let mentionedJid = [m.sender]
    conn.reply(m.chat, pesan, m, { contextInfo: { mentionedJid }})
