...
let handler = async (m, { conn, command, args }) => {
  ...
  count = count ? /all/i.test(count) ? Math.floor(global.DATABASE._data.users[m.sender].atm / xpperatm) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
  count = Math.max(1, count)
  if (global.DATABASE._data.users[m.sender].atm >= xpperatm * count) {
    global.DATABASE._data.users[m.sender].atm -= xpperatm * count
