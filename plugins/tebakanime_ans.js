...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakanime[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            global.DATABASE._data.users[m.sender].exp += this.tebakanime[id][2]
            await this.sendButton(m.chat, `*✅Benar!* +${this.tebakanime[id][2]} XP`, '© Mitsuha Botz', 'Tebak Anime', '.tebakanime')
