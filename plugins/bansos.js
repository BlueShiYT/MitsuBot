...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    global.DATABASE.data.users[m.sender].lastbansos = global.DATABASE._data.users[m.sender].lastbansos || 0
    ...
    if (new Date - global.DATABASE._data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        user.money -= 3000000
        global.DATABASE._data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 3000000
