...
let handler = async (m, { conn }) => {
    ...
    if (new Date - user.lastclaim > 86400000) {
        conn.reply(m.chat, `Anda sudah mengklaim dan mendapatkan 10000 💵money dan 🌟 30000 XP`, m)
...
handler.command = /^(parclaim|pardaily)$/i
handler.owner = false
handler.mods = false
...
function button(teks, user) {
    ...
    const buttonMessage = {
        contentText: teks,
        footerText: 'Gak Bisa Di Pake Kalo Daily Partner yang weekly dan monthly!',
        buttons: buttons,
        headerType: 1
    }
